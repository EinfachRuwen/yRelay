...
function requireAuth(req, res, next) {
  ...
  try {
    ...
    const user = db.prepare('SELECT id, username, email, display_name, role, is_active, has_seen_onboarding, ntfy_topic, email_notifications, has_schul_access, schul_poke_profile_id FROM users WHERE id = ?').get(decoded.userId);
